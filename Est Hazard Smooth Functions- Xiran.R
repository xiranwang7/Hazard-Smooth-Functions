...
############################# Read in Bone Data #############################
bone <- read.csv("/cloud/project/bone.csv") 
bone$time <- bone$t2
...
table1 <- smoothing_calculation_ep(data=bone, b=b, user_t=user_t, group='g')
table1 <- smoothing_calculation_ep(data=bone, b=b, user_t=user_t, group='')
...
cum_haz_plot <- function(data){ 
  ...
  t_i <- summary(fit)$time
  cumhaz <- summary(fit)$cumhaz
  plot(t_i,cumhaz, ylab="Cumulative Hazard", xlab = "Time", type="S", col="lightblue")
